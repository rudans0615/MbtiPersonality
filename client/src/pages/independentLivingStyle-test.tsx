...
export default function IndependentLivingStyleTest() {
  ...
  const handleAnswer = (points: number) => {
    ...
    setTimeout(() => {
      if (currentStep < independentLivingStyleQuestions.length - 1) {
        setCurrentStep(curr => curr + 1);
      } else {
        const totalScore = newHistory.reduce((a, b) => a + b, 0);
        setLocation("/independentLivingStyle-results?score=" + totalScore);
      }
    }, 300);
  };
  ...
  const question = independentLivingStyleQuestions[currentStep];
  const progress = Math.round(((currentStep + 1) / independentLivingStyleQuestions.length) * 100);
  ...
  return (
    <div className="min-h-screen bg-pink-50/30">
      <SEO title="너의 독립 공간 스타일은?" description="자취를 경험하며 나만의 공간을 꾸미는 일은 쉽지 않죠. 지금부터 12가지 상황을 통해 당신의 독립적인 공간 스타일을 알아봐요! 이 결과는 친구들과 공유하면 재밌을 거예요." url="https://mbtifinder.com/independentLivingStyle-test" keywords="PERSONALITY, 너의, 독립, 공간, 스타일은?" />
      <Navigation />
      {!hasStarted ? (
        <main className="flex-grow max-w-3xl mx-auto w-full px-4 py-12 pb-24 flex flex-col items-center">
          <div className="bg-white/90 backdrop-blur-sm rounded-[2rem] p-8 md:p-12 shadow-xl border border-white/60 flex-col justify-center text-center w-full mb-10">
            ...
            <h1 className="text-3xl md:text-5xl font-extrabold mb-6 tracking-tight text-neutral-900">너의 독립 공간 스타일은?</h1>
            <p className="text-lg md:text-xl text-neutral-500 mb-10 leading-relaxed">당신의 자취 공간은 어떤 분위기일까?</p>
          ...
          <div className="bg-white/60 backdrop-blur-sm border border-white/60 rounded-2xl p-8 text-left w-full mt-8 prose prose-neutral max-w-none">
            ...
            <p className="text-neutral-600 mb-4">너의 독립 공간 스타일은?는 당신의 심리를 깊이 있게 분석합니다. 자취를 경험하며 나만의 공간을 꾸미는 일은 쉽지 않죠. 지금부터 12가지 상황을 통해 당신의 독립적인 공간 스타일을 알아봐요! 이 결과는 친구들과 공유하면 재밌을 거예요.</p>
          ...
          <div className="bg-white/80 backdrop-blur-md shadow-sm sticky top-0 z-20">
            <div className="max-w-2xl mx-auto px-6 py-4">
              ...
              <p className="text-sm font-semibold tracking-wider text-pink-500 mt-3 text-center">
                {currentStep + 1} / {independentLivingStyleQuestions.length}
              </p>
            </div>
          </div>
              ...
              <div className="p-8 md:p-10 relative z-10">
                <div className="text-center mb-10">
                  ...
                  <h2 className="text-2xl md:text-[1.7rem] font-bold text-neutral-800 leading-snug tracking-tight break-keep">
                    {question?.question || question?.questionText || "너의 독립 공간 스타일은?"}
                  </h2>
                </div>
