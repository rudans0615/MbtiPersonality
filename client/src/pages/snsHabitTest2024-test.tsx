...
import { snsHabitTest2024Questions, seoArticle } from "@/data/snsHabitTest2024Questions";

export default function SnsHabitTest2024Test() {
  ...
  const handleAnswer = (points: number) => {
    ...
    setTimeout(() => {
      if (currentStep < snsHabitTest2024Questions.length - 1) {
        setCurrentStep(curr => curr + 1);
      } else {
        const totalScore = newHistory.reduce((a, b) => a + b, 0);
        setLocation("/snsHabitTest2024-results?score=" + totalScore);
      }
    }, 300);
  };
  ...
  const question = snsHabitTest2024Questions[currentStep];
  const progress = Math.round(((currentStep + 1) / snsHabitTest2024Questions.length) * 100);
  ...
  return (
    <div className="min-h-screen bg-pink-50/30">
      <SEO title="너의 SNS 중독도는 몇 %?" description="SNS가 생활의 일부가 된 요즘, 내 SNS 중독도는 얼마나 될까? 매일 손에서 스마트폰을 놓지 못하는 너, 직장에서의 대화 주제는 항상 SNS 이야기! 이 테스트를 통해 너의 SNS 습관을 체크해보자. 과연 얼마나 중독됐을까? 결과를 친구들과 공유하고 모두 함께 웃어봐!" url="https://mbtifinder.com/snsHabitTest2024-test" keywords="FUN, 너의, SNS, 중독도는, 몇, %?" />
      <Navigation />
      {!hasStarted ? (
        <main className="flex-grow max-w-3xl mx-auto w-full px-4 py-12 pb-24 flex flex-col items-center">
          <div className="bg-white/90 backdrop-blur-sm rounded-[2rem] p-8 md:p-12 shadow-xl border border-white/60 flex-col justify-center text-center w-full mb-10">
            ...
            <h1 className="text-3xl md:text-5xl font-extrabold mb-6 tracking-tight text-neutral-900">너의 SNS 중독도는 몇 %?</h1>
            <p className="text-lg md:text-xl text-neutral-500 mb-10 leading-relaxed">당신의 SNS 습관을 알아보자!</p>
          ...
          <div className="bg-white/60 backdrop-blur-sm border border-white/60 rounded-2xl p-8 text-left w-full mt-8 prose prose-neutral max-w-none">
            ...
            <p className="text-neutral-600 mb-4">너의 SNS 중독도는 몇 %?는 당신의 심리를 깊이 있게 분석합니다. SNS가 생활의 일부가 된 요즘, 내 SNS 중독도는 얼마나 될까? 매일 손에서 스마트폰을 놓지 못하는 너, 직장에서의 대화 주제는 항상 SNS 이야기! 이 테스트를 통해 너의 SNS 습관을 체크해보자. 과연 얼마나 중독됐을까? 결과를 친구들과 공유하고 모두 함께 웃어봐!</p>
          ...
          <div className="bg-white/80 backdrop-blur-md shadow-sm sticky top-0 z-20">
            <div className="max-w-2xl mx-auto px-6 py-4">
              ...
              <p className="text-sm font-semibold tracking-wider text-pink-500 mt-3 text-center">
                {currentStep + 1} / {snsHabitTest2024Questions.length}
              </p>
            </div>
          </div>
              ...
              <div className="p-8 md:p-10 relative z-10">
                <div className="text-center mb-10">
                  ...
                  <h2 className="text-2xl md:text-[1.7rem] font-bold text-neutral-800 leading-snug tracking-tight break-keep">
                    {question?.question || question?.questionText || "너의 SNS 중독도는 몇 %?"}
                  </h2>
                </div>
