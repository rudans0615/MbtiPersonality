...
import { socialMediaHabitTest2024Questions, seoArticle } from "@/data/socialMediaHabitTest2024Questions";

export default function SocialMediaHabitTest2024Test() {
  ...
  const handleAnswer = (points: number) => {
    ...
    setTimeout(() => {
      if (currentStep < socialMediaHabitTest2024Questions.length - 1) {
        setCurrentStep(curr => curr + 1);
      } else {
        const totalScore = newHistory.reduce((a, b) => a + b, 0);
        setLocation("/socialMediaHabitTest2024-results?score=" + totalScore);
      }
    }, 300);
  };
  ...
  const question = socialMediaHabitTest2024Questions[currentStep];
  const progress = Math.round(((currentStep + 1) / socialMediaHabitTest2024Questions.length) * 100);
  ...
  return (
    <div className="min-h-screen bg-pink-50/30">
      <SEO title="내가 어떤 SNS 중독자인지 알아보자!" description="내 SNS 사용 스타일을 파헤쳐 볼 시간! 친구와 함께 공유하고, 제일 잘 맞는 유형을 찾아보세요. 내 SNS 중독이 어떤 모습인지 재밌게 알아볼 수 있어요!" url="https://mbtifinder.com/socialMediaHabitTest2024-test" keywords="FUN, 내가, 어떤, SNS, 중독자인지, 알아보자!" />
      <Navigation />
      {!hasStarted ? (
        <main className="flex-grow max-w-3xl mx-auto w-full px-4 py-12 pb-24 flex flex-col items-center">
          <div className="bg-white/90 backdrop-blur-sm rounded-[2rem] p-8 md:p-12 shadow-xl border border-white/60 flex-col justify-center text-center w-full mb-10">
            ...
            <h1 className="text-3xl md:text-5xl font-extrabold mb-6 tracking-tight text-neutral-900">내가 어떤 SNS 중독자인지 알아보자!</h1>
            <p className="text-lg md:text-xl text-neutral-500 mb-10 leading-relaxed">소셜 미디어 사용 습관 진단해볼 사람?</p>
          ...
          <div className="bg-white/60 backdrop-blur-sm border border-white/60 rounded-2xl p-8 text-left w-full mt-8 prose prose-neutral max-w-none">
            ...
            <p className="text-neutral-600 mb-4">내가 어떤 SNS 중독자인지 알아보자!는 당신의 심리를 깊이 있게 분석합니다. 내 SNS 사용 스타일을 파헤쳐 볼 시간! 친구와 함께 공유하고, 제일 잘 맞는 유형을 찾아보세요. 내 SNS 중독이 어떤 모습인지 재밌게 알아볼 수 있어요!</p>
          ...
          <div className="bg-white/80 backdrop-blur-md shadow-sm sticky top-0 z-20">
            <div className="max-w-2xl mx-auto px-6 py-4">
              ...
              <p className="text-sm font-semibold tracking-wider text-pink-500 mt-3 text-center">
                {currentStep + 1} / {socialMediaHabitTest2024Questions.length}
              </p>
            </div>
          </div>
              ...
              <div className="p-8 md:p-10 relative z-10">
                <div className="text-center mb-10">
                  ...
                  <h2 className="text-2xl md:text-[1.7rem] font-bold text-neutral-800 leading-snug tracking-tight break-keep">
                    {question?.question || question?.questionText || "내가 어떤 SNS 중독자인지 알아보자!"}
                  </h2>
                </div>
