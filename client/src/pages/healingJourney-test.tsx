...
export default function HealingJourneyTest() {
  ...
  const handleAnswer = (points: number) => {
    ...
    setTimeout(() => {
      if (currentStep < healingJourneyQuestions.length - 1) {
        setCurrentStep(curr => curr + 1);
      } else {
        const totalScore = newHistory.reduce((a, b) => a + b, 0);
        setLocation("/healingJourney-results?score=" + totalScore);
      }
    }, 300);
  };
  ...
  const question = healingJourneyQuestions[currentStep];
  const progress = Math.round(((currentStep + 1) / healingJourneyQuestions.length) * 100);
  ...
  return (
    <div className="min-h-screen bg-pink-50/30">
      <SEO title="당신의 힐링 여행 스타일은?" description="이 심리 테스트를 통해 나만의 완벽한 힐링 여행 스타일을 알아보세요! 어떤 여행이 마음의 번아웃에서 벗어나게 해줄지 궁금하지 않나요? 친구들과 함께 공유하고 다 같이 힐링해요!" url="https://mbtifinder.com/healingJourney-test" keywords="PERSONALITY, 당신의, 힐링, 여행, 스타일은?" />
      <Navigation />
      {!hasStarted ? (
        <main className="flex-grow max-w-3xl mx-auto w-full px-4 py-12 pb-24 flex flex-col items-center">
          <div className="bg-white/90 backdrop-blur-sm rounded-[2rem] p-8 md:p-12 shadow-xl border border-white/60 flex-col justify-center text-center w-full mb-10">
            ...
            <h1 className="text-3xl md:text-5xl font-extrabold mb-6 tracking-tight text-neutral-900">당신의 힐링 여행 스타일은?</h1>
            <p className="text-lg md:text-xl text-neutral-500 mb-10 leading-relaxed">어떤 여행이 당신의 마음을 치유할까?</p>
          ...
          <div className="bg-white/60 backdrop-blur-sm border border-white/60 rounded-2xl p-8 text-left w-full mt-8 prose prose-neutral max-w-none">
            ...
            <p className="text-neutral-600 mb-4">당신의 힐링 여행 스타일은?는 당신의 심리를 깊이 있게 분석합니다. 이 심리 테스트를 통해 나만의 완벽한 힐링 여행 스타일을 알아보세요! 어떤 여행이 마음의 번아웃에서 벗어나게 해줄지 궁금하지 않나요? 친구들과 함께 공유하고 다 같이 힐링해요!</p>
          ...
          <div className="bg-white/80 backdrop-blur-md shadow-sm sticky top-0 z-20">
            <div className="max-w-2xl mx-auto px-6 py-4">
              ...
              <p className="text-sm font-semibold tracking-wider text-pink-500 mt-3 text-center">
                {currentStep + 1} / {healingJourneyQuestions.length}
              </p>
            </div>
          </div>
              ...
              <div className="p-8 md:p-10 relative z-10">
                <div className="text-center mb-10">
                  ...
                  <h2 className="text-2xl md:text-[1.7rem] font-bold text-neutral-800 leading-snug tracking-tight break-keep">
                    {question?.question || question?.questionText || "당신의 힐링 여행 스타일은?"}
                  </h2>
                </div>
