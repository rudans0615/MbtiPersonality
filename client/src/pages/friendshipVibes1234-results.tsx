...
import { calculateFriendshipVibes1234TestLevel, friendshipVibes1234Results } from "@/data/friendshipVibes1234Types";
import { friendshipVibes1234Questions } from "@/data/friendshipVibes1234Questions";
import { testTypes } from "@/data/testTypes";
import { Loader2, RotateCcw, Zap, Sparkles } from "lucide-react";

export default function FriendshipVibes1234Results() {
    ...
    const timer = setTimeout(() => {
      ...
      if (typeof window !== "undefined" && (window as any).gtag) {
        (window as any).gtag('event', 'test_completed', {
          test_id: "friendshipVibes1234",
          options_count: friendshipVibes1234Questions[0]?.options?.length || 2,
          questions_count: friendshipVibes1234Questions.length
        });
      }
    }, 3000);
  ...
  const resultKey = calculateFriendshipVibes1234TestLevel(score);
  const allKeys = Object.keys(friendshipVibes1234Results);
  const result = friendshipVibes1234Results[resultKey] || friendshipVibes1234Results[allKeys[0]] || { title: "분석 완료", description: "당신만의 특별한 결과입니다!" };

  // Cross-link: random 3 other tests
  const otherTests = testTypes.filter(t => t.id !== "friendshipVibes1234" && t.isAvailable).sort(() => Math.random() - 0.5).slice(0, 3);

  if (isAnalyzing) {
    return (
      <div className="min-h-screen bg-pink-50/30 flex flex-col font-sans">
        <SEO title="너의 우정 스타일을 알아보자! - 결과 분석 중" />
        <Navigation />
  ...
  return (
    <div className="min-h-screen bg-pink-50/30">
      <SEO title={(result.title || "결과") + " | 너의 우정 스타일을 알아보자!"} description={result.description} url={`https://mbtifinder.com/friendshipVibes1234-results?score=${score}`} />
      <Navigation />
      ...
      <div className="bg-gradient-to-br from-pink-300 via-purple-300 to-indigo-300 text-white py-20 relative overflow-hidden">
        <div className="max-w-3xl mx-auto px-6 text-center relative z-10">
          <h1 className="text-xl md:text-2xl font-semibold mb-4 tracking-wider opacity-90 drop-shadow-md">너의 우정 스타일을 알아보자! 결과 💫</h1>
      ...
      <div className="max-w-3xl mx-auto px-6 py-12">
        <div className="bg-white/80 backdrop-blur-xl border border-white/60 rounded-[2.5rem] shadow-[0_8px_32px_rgba(236,72,153,0.1)] p-8 md:p-12 -mt-24 relative z-20 overflow-hidden">
          ...
          <div className="text-center space-y-5 mt-10">
            <ShareButtons title="너의 우정 스타일을 알아보자! 결과" shareText={`나의 검사 결과는 '${result.title}'! 너도 해봐 👉`} url="https://mbtifinder.com/friendshipVibes1234-test" />
          </div>

          <div className="mt-8 flex justify-center pb-4">
            <Button onClick={() => { window.location.href = "/friendshipVibes1234-test"; }} className="bg-gradient-to-r from-pink-400 via-purple-400 to-indigo-400 text-white px-10 py-6 rounded-2xl text-[1.1rem] font-bold hover:shadow-lg transition-transform hover:-translate-y-1 border-2 border-white/50 w-full">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {allKeys.map((key) => {
            const type = friendshipVibes1234Results[key];
            return (
