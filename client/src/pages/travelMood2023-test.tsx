...
import { travelMood2023Questions, seoArticle } from "@/data/travelMood2023Questions";

export default function TravelMood2023Test() {
  ...
  const handleAnswer = (points: number) => {
    ...
    setTimeout(() => {
      if (currentStep < travelMood2023Questions.length - 1) {
        setCurrentStep(curr => curr + 1);
      } else {
        const totalScore = newHistory.reduce((a, b) => a + b, 0);
        setLocation("/travelMood2023-results?score=" + totalScore);
      }
    }, 300);
  };
  ...
  const question = travelMood2023Questions[currentStep];
  const progress = Math.round(((currentStep + 1) / travelMood2023Questions.length) * 100);
  ...
  return (
    <div className="min-h-screen bg-pink-50/30">
      <SEO title="지금 당신의 여행 스타일은?" description="바쁜 일상 속에서 잠깐의 여유와 힐링이 필요한 순간! 내 여행 스타일을 알아보고, 진정한 나만의 휴식법을 찾아보세요. 너무 다양한 방법들이 있으니, 당신에게 꼭 맞는 여행 방법을 발견하는 재미를 느껴보세요!" url="https://mbtifinder.com/travelMood2023-test" keywords="FUN, 지금, 당신의, 여행, 스타일은?" />
      <Navigation />
      {!hasStarted ? (
        <main className="flex-grow max-w-3xl mx-auto w-full px-4 py-12 pb-24 flex flex-col items-center">
          <div className="bg-white/90 backdrop-blur-sm rounded-[2rem] p-8 md:p-12 shadow-xl border border-white/60 flex-col justify-center text-center w-full mb-10">
            ...
            <h1 className="text-3xl md:text-5xl font-extrabold mb-6 tracking-tight text-neutral-900">지금 당신의 여행 스타일은?</h1>
            <p className="text-lg md:text-xl text-neutral-500 mb-10 leading-relaxed">내가 찾는 힐링의 순간은 어떤 걸까?</p>
          ...
          <div className="bg-white/60 backdrop-blur-sm border border-white/60 rounded-2xl p-8 text-left w-full mt-8 prose prose-neutral max-w-none">
            ...
            <p className="text-neutral-600 mb-4">지금 당신의 여행 스타일은?는 당신의 심리를 깊이 있게 분석합니다. 바쁜 일상 속에서 잠깐의 여유와 힐링이 필요한 순간! 내 여행 스타일을 알아보고, 진정한 나만의 휴식법을 찾아보세요. 너무 다양한 방법들이 있으니, 당신에게 꼭 맞는 여행 방법을 발견하는 재미를 느껴보세요!</p>
          ...
          <div className="bg-white/80 backdrop-blur-md shadow-sm sticky top-0 z-20">
            <div className="max-w-2xl mx-auto px-6 py-4">
              ...
              <p className="text-sm font-semibold tracking-wider text-pink-500 mt-3 text-center">
                {currentStep + 1} / {travelMood2023Questions.length}
              </p>
            </div>
          </div>
              ...
              <div className="p-8 md:p-10 relative z-10">
                <div className="text-center mb-10">
                  ...
                  <h2 className="text-2xl md:text-[1.7rem] font-bold text-neutral-800 leading-snug tracking-tight break-keep">
                    {question?.question || question?.questionText || "지금 당신의 여행 스타일은?"}
                  </h2>
                </div>
