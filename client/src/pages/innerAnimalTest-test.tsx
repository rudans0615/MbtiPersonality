...
import { innerAnimalTestQuestions } from "@/data/innerAnimalTestQuestions";

export default function InnerAnimalTestTest() {
  ...
  const handleAnswer = (points: number) => {
    ...
    if (currentStep < innerAnimalTestQuestions.length - 1) {
      setCurrentStep(curr => curr + 1);
    } else {
      const totalScore = newHistory.reduce((a, b) => a + b, 0);
      setLocation("/innerAnimalTest-results?score=" + totalScore);
    }
  };
  ...
  const question = innerAnimalTestQuestions[currentStep];
  const progress = Math.round(((currentStep + 1) / innerAnimalTestQuestions.length) * 100);

  return (
    <div className="min-h-screen bg-[#F8F9FB] flex flex-col font-sans">
      <SEO title="당신의 마음속 동물은 무엇인가요?" description="고유한 성격과 숨겨진 본능을 가지고 있는 당신의 마음속 동물은 과연 어떤 모습일까요? 이 테스트를 통해 당신이 어떤 동물의 특성을 지니고 있는지 알아보세요! 당신의 내면을 들여다보며 새로운 스스로를 발견하는 기회를 놓치지 마세요." url="https://mbtifinder.com/innerAnimalTest-test" />
      <Navigation />
      {!hasStarted ? (
        <main className="flex-grow max-w-3xl mx-auto w-full px-4 py-12 pb-24 flex flex-col items-center">
          <div className="bg-white rounded-[2rem] p-8 md:p-12 shadow-md flex-col justify-center text-center w-full mb-10 border border-neutral-100">
            ...
            <h1 className="text-3xl md:text-5xl font-extrabold mb-6 tracking-tight text-neutral-900">당신의 마음속 동물은 무엇인가요?</h1>
            <p className="text-lg md:text-xl text-neutral-500 mb-10 leading-relaxed">심리 테스트로 숨겨진 본성을 발견하세요!</p>
          ...
          <div className="bg-white/60 border border-neutral-200 rounded-2xl p-8 text-left w-full mt-8 prose prose-neutral max-w-none">
            ...
            <p className="text-neutral-600 mb-4">당신의 마음속 동물은 무엇인가요?는 당신의 심리를 깊이 있게 분석합니다. 고유한 성격과 숨겨진 본능을 가지고 있는 당신의 마음속 동물은 과연 어떤 모습일까요? 이 테스트를 통해 당신이 어떤 동물의 특성을 지니고 있는지 알아보세요! 당신의 내면을 들여다보며 새로운 스스로를 발견하는 기회를 놓치지 마세요.</p>
        ...
        <main className="flex-grow max-w-2xl mx-auto w-full px-4 py-8 flex flex-col">
          <div className="mb-8">
            <div className="flex justify-between text-sm font-bold text-neutral-500 mb-2 px-2">
              <span>진행률</span>
              <span>{currentStep + 1} / {innerAnimalTestQuestions.length}</span>
            </div>
          ...
          <div className="bg-white rounded-3xl p-8 md:p-10 shadow-lg flex-grow flex flex-col justify-center text-center">
            <h2 className="text-2xl md:text-3xl font-bold mb-10 leading-relaxed text-neutral-800 break-keep">{question?.question || question?.questionText || "당신의 마음속 동물은 무엇인가요?"}</h2>
