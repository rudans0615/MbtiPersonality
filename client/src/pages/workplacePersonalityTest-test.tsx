...
import { workplacePersonalityTestQuestions } from "@/data/workplacePersonalityTestQuestions";

export default function WorkplacePersonalityTestTest() {
  ...
  const handleAnswer = (points: number) => {
    ...
    setTimeout(() => {
      if (currentStep < workplacePersonalityTestQuestions.length - 1) {
        setCurrentStep(curr => curr + 1);
      } else {
        const totalScore = newHistory.reduce((a, b) => a + b, 0);
        setLocation("/workplacePersonalityTest-results?score=" + totalScore);
      }
    }, 300);
  };
  ...
  const question = workplacePersonalityTestQuestions[currentStep];
  const progress = Math.round(((currentStep + 1) / workplacePersonalityTestQuestions.length) * 100);
  ...
  return (
    <div className="min-h-screen bg-pink-50/30">
      <SEO title="너의 직장 생활 뚜껑 열어보기!" description="직장에서의 경험과 너의 성향은 어떤 직장인의 모습을 만들어 주는 걸까? 야근 후 퇴사 결정까지, 다양한 상황 속에서 널 드러내는 심리 테스트를 해봐." url="https://mbtifinder.com/workplacePersonalityTest-test" keywords="CAREER, 너의, 직장, 생활, 뚜껑, 열어보기!" />
      <Navigation />
      {!hasStarted ? (
        <main className="flex-grow max-w-3xl mx-auto w-full px-4 py-12 pb-24 flex flex-col items-center">
          <div className="bg-white/90 backdrop-blur-sm rounded-[2rem] p-8 md:p-12 shadow-xl border border-white/60 flex-col justify-center text-center w-full mb-10">
            ...
            <h1 className="text-3xl md:text-5xl font-extrabold mb-6 tracking-tight text-neutral-900">너의 직장 생활 뚜껑 열어보기!</h1>
            <p className="text-lg md:text-xl text-neutral-500 mb-10 leading-relaxed">퇴사 후의 넷플릭스, 찐 염원을 확인해봐!</p>
          ...
          <div className="bg-white/60 backdrop-blur-sm border border-white/60 rounded-2xl p-8 text-left w-full mt-8 prose prose-neutral max-w-none">
            ...
            <p className="text-neutral-600 mb-4">너의 직장 생활 뚜껑 열어보기!는 당신의 심리를 깊이 있게 분석합니다. 직장에서의 경험과 너의 성향은 어떤 직장인의 모습을 만들어 주는 걸까? 야근 후 퇴사 결정까지, 다양한 상황 속에서 널 드러내는 심리 테스트를 해봐.</p>
            <p className="text-neutral-600 mb-6">총 5개의 문항으로 이루어져 있으며, 직관적으로 가장 먼저 떠오르는 답변을 선택하는 것이 가장 정확합니다.</p>
          </div>
          ...
          <div className="bg-white/80 backdrop-blur-md shadow-sm sticky top-0 z-20">
            <div className="max-w-2xl mx-auto px-6 py-4">
              ...
              <p className="text-sm font-semibold tracking-wider text-pink-500 mt-3 text-center">
                {currentStep + 1} / {workplacePersonalityTestQuestions.length}
              </p>
            </div>
          </div>
              ...
              <div className="p-8 md:p-10 relative z-10">
                <div className="text-center mb-10">
                  ...
                  <h2 className="text-2xl md:text-[1.7rem] font-bold text-neutral-800 leading-snug tracking-tight break-keep">
                    {question?.question || question?.questionText || "너의 직장 생활 뚜껑 열어보기!"}
                  </h2>
                </div>
