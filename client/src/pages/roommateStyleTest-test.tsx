...
import { roommateStyleTestQuestions } from "@/data/roommateStyleTestQuestions";

export default function RoommateStyleTestTest() {
  ...
  const handleAnswer = (points: number) => {
    ...
    // Q1 interstitial: show "analyzing" screen with ad
    if (currentStep === 1 - 1) {
    ...
    setTimeout(() => {
      if (currentStep < roommateStyleTestQuestions.length - 1) {
        setCurrentStep(curr => curr + 1);
      } else {
        const totalScore = newHistory.reduce((a, b) => a + b, 0);
        setLocation("/roommateStyleTest-results?score=" + totalScore);
      }
    }, 300);
  };
  ...
  const question = roommateStyleTestQuestions[currentStep];
  const progress = Math.round(((currentStep + 1) / roommateStyleTestQuestions.length) * 100);
  ...
  return (
    <div className="min-h-screen bg-pink-50/30">
      <SEO title="나의 룸메 스타일은?" description="각양각색의 룸메이트와 함께하는 자취 생활. 너와 잘 맞는 룸메 스타일은 어떤 걸까? 아래 질문에 답해보면서, 내 스타일과 잘 맞는 룸메를 찾아보자!" url="https://mbtifinder.com/roommateStyleTest-test" keywords="PERSONALITY, 나의, 룸메, 스타일은?" />
      <Navigation />
      {!hasStarted ? (
        <main className="flex-grow max-w-3xl mx-auto w-full px-4 py-12 pb-24 flex flex-col items-center">
          <div className="bg-white/90 backdrop-blur-sm rounded-[2rem] p-8 md:p-12 shadow-xl border border-white/60 flex-col justify-center text-center w-full mb-10">
            ...
            <h1 className="text-3xl md:text-5xl font-extrabold mb-6 tracking-tight text-neutral-900">나의 룸메 스타일은?</h1>
            <p className="text-lg md:text-xl text-neutral-500 mb-10 leading-relaxed">너의 라이프스타일에 맞는 룸메이트 유형 찾기!</p>
          ...
          <div className="bg-white/60 backdrop-blur-sm border border-white/60 rounded-2xl p-8 text-left w-full mt-8 prose prose-neutral max-w-none">
            ...
            <p className="text-neutral-600 mb-4">나의 룸메 스타일은?는 당신의 심리를 깊이 있게 분석합니다. 각양각색의 룸메이트와 함께하는 자취 생활. 너와 잘 맞는 룸메 스타일은 어떤 걸까? 아래 질문에 답해보면서, 내 스타일과 잘 맞는 룸메를 찾아보자!</p>
            <p className="text-neutral-600 mb-6">총 3개의 문항으로 이루어져 있으며, 직관적으로 가장 먼저 떠오르는 답변을 선택하는 것이 가장 정확합니다.</p>
          </div>
          ...
          <div className="bg-white/80 backdrop-blur-md shadow-sm sticky top-0 z-20">
            <div className="max-w-2xl mx-auto px-6 py-4">
              ...
              <p className="text-sm font-semibold tracking-wider text-pink-500 mt-3 text-center">
                {currentStep + 1} / {roommateStyleTestQuestions.length}
              </p>
            </div>
          </div>
              ...
              <div className="p-8 md:p-10 relative z-10">
                <div className="text-center mb-10">
                  ...
                  <h2 className="text-2xl md:text-[1.7rem] font-bold text-neutral-800 leading-snug tracking-tight break-keep">
                    {question?.question || question?.questionText || "나의 룸메 스타일은?"}
                  </h2>
                </div>
