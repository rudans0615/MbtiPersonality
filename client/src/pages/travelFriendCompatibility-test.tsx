...
export default function TravelFriendCompatibilityTest() {
  ...
  const handleAnswer = (points: number) => {
    ...
    setTimeout(() => {
      if (currentStep < travelFriendCompatibilityQuestions.length - 1) {
        setCurrentStep(curr => curr + 1);
      } else {
        const totalScore = newHistory.reduce((a, b) => a + b, 0);
        setLocation("/travelFriendCompatibility-results?score=" + totalScore);
      }
    }, 300);
  };
  ...
  const question = travelFriendCompatibilityQuestions[currentStep];
  const progress = Math.round(((currentStep + 1) / travelFriendCompatibilityQuestions.length) * 100);
  ...
  return (
    <div className="min-h-screen bg-pink-50/30">
      <SEO title="내 MBTI로 알아보는 최애 여행지 친구 궁합✈️" description="내 MBTI 성향에 따라 최애 여행지와 그곳에서 잘 맞는 친구 유형을 알아보자! 여행을 떠나고 싶은 그날, 어떤 친구와 함께하면 좋을지 체크해보세요!" url="https://mbtifinder.com/travelFriendCompatibility-test" keywords="FUN, 내, MBTI로, 알아보는, 최애, 여행지, 친구, 궁합✈️" />
      <Navigation />
      {!hasStarted ? (
        <main className="flex-grow max-w-3xl mx-auto w-full px-4 py-12 pb-24 flex flex-col items-center">
          <div className="bg-white/90 backdrop-blur-sm rounded-[2rem] p-8 md:p-12 shadow-xl border border-white/60 flex-col justify-center text-center w-full mb-10">
            ...
            <h1 className="text-3xl md:text-5xl font-extrabold mb-6 tracking-tight text-neutral-900">내 MBTI로 알아보는 최애 여행지 친구 궁합✈️</h1>
            <p className="text-lg md:text-xl text-neutral-500 mb-10 leading-relaxed">너의 성격에 맞는 여행지와 친구의 조화는?</p>
          ...
          <div className="bg-white/60 backdrop-blur-sm border border-white/60 rounded-2xl p-8 text-left w-full mt-8 prose prose-neutral max-w-none">
            ...
            <p className="text-neutral-600 mb-4">내 MBTI로 알아보는 최애 여행지 친구 궁합✈️는 당신의 심리를 깊이 있게 분석합니다. 내 MBTI 성향에 따라 최애 여행지와 그곳에서 잘 맞는 친구 유형을 알아보자! 여행을 떠나고 싶은 그날, 어떤 친구와 함께하면 좋을지 체크해보세요!</p>
          ...
          <div className="bg-white/80 backdrop-blur-md shadow-sm sticky top-0 z-20">
            <div className="max-w-2xl mx-auto px-6 py-4">
              ...
              <p className="text-sm font-semibold tracking-wider text-pink-500 mt-3 text-center">
                {currentStep + 1} / {travelFriendCompatibilityQuestions.length}
              </p>
            </div>
          </div>
              ...
              <div className="p-8 md:p-10 relative z-10">
                <div className="text-center mb-10">
                  ...
                  <h2 className="text-2xl md:text-[1.7rem] font-bold text-neutral-800 leading-snug tracking-tight break-keep">
                    {question?.question || question?.questionText || "내 MBTI로 알아보는 최애 여행지 친구 궁합✈️"}
                  </h2>
                </div>
