...
import { foodPreference2023Questions, seoArticle } from "@/data/foodPreference2023Questions";

export default function FoodPreference2023Test() {
  ...
  const handleAnswer = (points: number) => {
    ...
    setTimeout(() => {
      if (currentStep < foodPreference2023Questions.length - 1) {
        setCurrentStep(curr => curr + 1);
      } else {
        const totalScore = newHistory.reduce((a, b) => a + b, 0);
        setLocation("/foodPreference2023-results?score=" + totalScore);
      }
    }, 300);
  };
  ...
  const question = foodPreference2023Questions[currentStep];
  const progress = Math.round(((currentStep + 1) / foodPreference2023Questions.length) * 100);
  ...
  return (
    <div className="min-h-screen bg-pink-50/30">
      <SEO title="너의 배달앱 최애 메뉴는?" description="배달의 민족, 요기요, 또는 쿠팡이츠에서 어떤 메뉴를 가장 많이 시켜 먹는지 궁금하지? 이 테스트를 통해 너의 먹방 취향을 알아보자! 친구들과 공유하고 나와 비슷한 취향인 사람들을 찾아보는 것도 재밌을 거야!" url="https://mbtifinder.com/foodPreference2023-test" keywords="FUN, 너의, 배달앱, 최애, 메뉴는?" />
      <Navigation />
      {!hasStarted ? (
        <main className="flex-grow max-w-3xl mx-auto w-full px-4 py-12 pb-24 flex flex-col items-center">
          <div className="bg-white/90 backdrop-blur-sm rounded-[2rem] p-8 md:p-12 shadow-xl border border-white/60 flex-col justify-center text-center w-full mb-10">
            ...
            <h1 className="text-3xl md:text-5xl font-extrabold mb-6 tracking-tight text-neutral-900">너의 배달앱 최애 메뉴는?</h1>
            <p className="text-lg md:text-xl text-neutral-500 mb-10 leading-relaxed">당신의 진짜 취향을 파헤쳐보자!</p>
          ...
          <div className="bg-white/60 backdrop-blur-sm border border-white/60 rounded-2xl p-8 text-left w-full mt-8 prose prose-neutral max-w-none">
            ...
            <p className="text-neutral-600 mb-4">너의 배달앱 최애 메뉴는?는 당신의 심리를 깊이 있게 분석합니다. 배달의 민족, 요기요, 또는 쿠팡이츠에서 어떤 메뉴를 가장 많이 시켜 먹는지 궁금하지? 이 테스트를 통해 너의 먹방 취향을 알아보자! 친구들과 공유하고 나와 비슷한 취향인 사람들을 찾아보는 것도 재밌을 거야!</p>
          ...
          <div className="bg-white/80 backdrop-blur-md shadow-sm sticky top-0 z-20">
            <div className="max-w-2xl mx-auto px-6 py-4">
              ...
              <p className="text-sm font-semibold tracking-wider text-pink-500 mt-3 text-center">
                {currentStep + 1} / {foodPreference2023Questions.length}
              </p>
            </div>
          </div>
              ...
              <div className="p-8 md:p-10 relative z-10">
                <div className="text-center mb-10">
                  ...
                  <h2 className="text-2xl md:text-[1.7rem] font-bold text-neutral-800 leading-snug tracking-tight break-keep">
                    {question?.question || question?.questionText || "너의 배달앱 최애 메뉴는?"}
                  </h2>
                </div>
