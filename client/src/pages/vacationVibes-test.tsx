...
import { vacationVibesQuestions } from "@/data/vacationVibesQuestions";

export default function VacationVibesTest() {
  ...
  const handleAnswer = (points: number) => {
    ...
    setTimeout(() => {
      if (currentStep < vacationVibesQuestions.length - 1) {
        setCurrentStep(curr => curr + 1);
      } else {
        const totalScore = newHistory.reduce((a, b) => a + b, 0);
        setLocation("/vacationVibes-results?score=" + totalScore);
      }
    }, 300);
  };
  ...
  const question = vacationVibesQuestions[currentStep];
  const progress = Math.round(((currentStep + 1) / vacationVibesQuestions.length) * 100);
  ...
  return (
    <div className="min-h-screen bg-pink-50/30">
      <SEO title="너의 휴식 스타일은?" description="이번 심리테스트로 나만의 휴식 스타일을 알아보자! 좋아하는 여행 스타일에 따라 내가 진정으로 필요로 하는 힐링 방법을 찾아보세요." url="https://mbtifinder.com/vacationVibes-test" keywords="FUN, 너의, 휴식, 스타일은?" />
      <Navigation />
      {!hasStarted ? (
        <main className="flex-grow max-w-3xl mx-auto w-full px-4 py-12 pb-24 flex flex-col items-center">
          <div className="bg-white/90 backdrop-blur-sm rounded-[2rem] p-8 md:p-12 shadow-xl border border-white/60 flex-col justify-center text-center w-full mb-10">
            ...
            <h1 className="text-3xl md:text-5xl font-extrabold mb-6 tracking-tight text-neutral-900">너의 휴식 스타일은?</h1>
            <p className="text-lg md:text-xl text-neutral-500 mb-10 leading-relaxed">어떤 여행지를 떠나야 할까?</p>
          ...
          <div className="bg-white/60 backdrop-blur-sm border border-white/60 rounded-2xl p-8 text-left w-full mt-8 prose prose-neutral max-w-none">
            ...
            <p className="text-neutral-600 mb-4">너의 휴식 스타일은?는 당신의 심리를 깊이 있게 분석합니다. 이번 심리테스트로 나만의 휴식 스타일을 알아보자! 좋아하는 여행 스타일에 따라 내가 진정으로 필요로 하는 힐링 방법을 찾아보세요.</p>
          ...
          <div className="bg-white/80 backdrop-blur-md shadow-sm sticky top-0 z-20">
            <div className="max-w-2xl mx-auto px-6 py-4">
              ...
              <p className="text-sm font-semibold tracking-wider text-pink-500 mt-3 text-center">
                {currentStep + 1} / {vacationVibesQuestions.length}
              </p>
            </div>
          </div>
              ...
              <div className="p-8 md:p-10 relative z-10">
                <div className="text-center mb-10">
                  ...
                  <h2 className="text-2xl md:text-[1.7rem] font-bold text-neutral-800 leading-snug tracking-tight break-keep">
                    {question?.question || question?.questionText || "너의 휴식 스타일은?"}
                  </h2>
                </div>
