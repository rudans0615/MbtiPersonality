...
import { shoppingMoodTest1234Questions, seoArticle } from "@/data/shoppingMoodTest1234Questions";

export default function ShoppingMoodTest1234Test() {
  ...
  const handleAnswer = (points: number) => {
    ...
    setTimeout(() => {
      if (currentStep < shoppingMoodTest1234Questions.length - 1) {
        setCurrentStep(curr => curr + 1);
      } else {
        const totalScore = newHistory.reduce((a, b) => a + b, 0);
        setLocation("/shoppingMoodTest1234-results?score=" + totalScore);
      }
    }, 300);
  };
  ...
  const question = shoppingMoodTest1234Questions[currentStep];
  const progress = Math.round(((currentStep + 1) / shoppingMoodTest1234Questions.length) * 100);
  ...
  return (
    <div className="min-h-screen bg-pink-50/30">
      <SEO title="당신의 쇼핑 기분은 어떤 타입?" description="너의 쇼핑 기분을 알아보자! 어떤 상황에서 소비를 하게 되면 나의 진짜 감정이 드러나는 걸까? 이 테스트를 통해 네가 어떤 소비 습관을 가지고 있는지, 그리고 그 기분에 따라 어떤 행동을 하는지 확인해보자. 친구들과 함께 해보고, 서로의 쇼핑 타입을 비교해보는 것도 재미있을 거야!" url="https://mbtifinder.com/shoppingMoodTest1234-test" keywords="FUN, 당신의, 쇼핑, 기분은, 어떤, 타입?" />
      <Navigation />
      {!hasStarted ? (
        <main className="flex-grow max-w-3xl mx-auto w-full px-4 py-12 pb-24 flex flex-col items-center">
          <div className="bg-white/90 backdrop-blur-sm rounded-[2rem] p-8 md:p-12 shadow-xl border border-white/60 flex-col justify-center text-center w-full mb-10">
            ...
            <h1 className="text-3xl md:text-5xl font-extrabold mb-6 tracking-tight text-neutral-900">당신의 쇼핑 기분은 어떤 타입?</h1>
            <p className="text-lg md:text-xl text-neutral-500 mb-10 leading-relaxed">소비를 통해 드러나는 내 마음의 소리</p>
          ...
          <div className="bg-white/60 backdrop-blur-sm border border-white/60 rounded-2xl p-8 text-left w-full mt-8 prose prose-neutral max-w-none">
            ...
            <p className="text-neutral-600 mb-4">당신의 쇼핑 기분은 어떤 타입?는 당신의 심리를 깊이 있게 분석합니다. 너의 쇼핑 기분을 알아보자! 어떤 상황에서 소비를 하게 되면 나의 진짜 감정이 드러나는 걸까? 이 테스트를 통해 네가 어떤 소비 습관을 가지고 있는지, 그리고 그 기분에 따라 어떤 행동을 하는지 확인해보자. 친구들과 함께 해보고, 서로의 쇼핑 타입을 비교해보는 것도 재미있을 거야!</p>
          ...
          <div className="bg-white/80 backdrop-blur-md shadow-sm sticky top-0 z-20">
            <div className="max-w-2xl mx-auto px-6 py-4">
              ...
              <p className="text-sm font-semibold tracking-wider text-pink-500 mt-3 text-center">
                {currentStep + 1} / {shoppingMoodTest1234Questions.length}
              </p>
            </div>
          </div>
              ...
              <div className="p-8 md:p-10 relative z-10">
                <div className="text-center mb-10">
                  ...
                  <h2 className="text-2xl md:text-[1.7rem] font-bold text-neutral-800 leading-snug tracking-tight break-keep">
                    {question?.question || question?.questionText || "당신의 쇼핑 기분은 어떤 타입?"}
                  </h2>
                </div>
