...
import { calculateTravelMoodCheckerTestLevel, travelMoodCheckerResults } from "@/data/travelMoodCheckerTypes";
import { travelMoodCheckerQuestions } from "@/data/travelMoodCheckerQuestions";
import { testTypes } from "@/data/testTypes";
import { Loader2, RotateCcw, Zap, Sparkles } from "lucide-react";

export default function TravelMoodCheckerResults() {
    ...
    const timer = setTimeout(() => {
      ...
      if (typeof window !== "undefined" && (window as any).gtag) {
        (window as any).gtag('event', 'test_completed', {
          test_id: "travelMoodChecker",
          options_count: travelMoodCheckerQuestions[0]?.options?.length || 2,
          questions_count: travelMoodCheckerQuestions.length
        });
      }
    }, 3000);
  ...
  const resultKey = calculateTravelMoodCheckerTestLevel(score);
  const allKeys = Object.keys(travelMoodCheckerResults);
  const result = travelMoodCheckerResults[resultKey] || travelMoodCheckerResults[allKeys[0]] || { title: "분석 완료", description: "당신만의 특별한 결과입니다!" };

  // Cross-link: random 3 other tests
  const otherTests = testTypes.filter(t => t.id !== "travelMoodChecker" && t.isAvailable).sort(() => Math.random() - 0.5).slice(0, 3);

  if (isAnalyzing) {
    return (
      <div className="min-h-screen bg-pink-50/30 flex flex-col font-sans">
        <SEO title="내 여행 스타일은 어떤 걸까? - 결과 분석 중" />
        <Navigation />
  ...
  return (
    <div className="min-h-screen bg-pink-50/30">
      <SEO title={(result.title || "결과") + " | 내 여행 스타일은 어떤 걸까?"} description={result.description} url={`https://mbtifinder.com/travelMoodChecker-results?score=${score}`} />
      <Navigation />
      ...
      <div className="bg-gradient-to-br from-pink-300 via-purple-300 to-indigo-300 text-white py-20 relative overflow-hidden">
        <div className="max-w-3xl mx-auto px-6 text-center relative z-10">
          <h1 className="text-xl md:text-2xl font-semibold mb-4 tracking-wider opacity-90 drop-shadow-md">내 여행 스타일은 어떤 걸까? 결과 💫</h1>
      ...
      <div className="max-w-3xl mx-auto px-6 py-12">
        <div className="bg-white/80 backdrop-blur-xl border border-white/60 rounded-[2.5rem] shadow-[0_8px_32px_rgba(236,72,153,0.1)] p-8 md:p-12 -mt-24 relative z-20 overflow-hidden">
          ...
          <div className="text-center space-y-5 mt-10">
            <ShareButtons title="내 여행 스타일은 어떤 걸까? 결과" shareText={`나의 검사 결과는 '${result.title}'! 너도 해봐 👉`} url="https://mbtifinder.com/travelMoodChecker-test" />
          </div>

          <div className="mt-8 flex justify-center pb-4">
            <Button onClick={() => { window.location.href = "/travelMoodChecker-test"; }} className="bg-gradient-to-r from-pink-400 via-purple-400 to-indigo-400 text-white px-10 py-6 rounded-2xl text-[1.1rem] font-bold hover:shadow-lg transition-transform hover:-translate-y-1 border-2 border-white/50 w-full">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {allKeys.map((key) => {
            const type = travelMoodCheckerResults[key];
            return (
