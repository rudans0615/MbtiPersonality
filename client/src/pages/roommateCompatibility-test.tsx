...
export default function RoommateCompatibilityTest() {
  ...
  const handleAnswer = (points: number) => {
    ...
    setTimeout(() => {
      if (currentStep < roommateCompatibilityQuestions.length - 1) {
        setCurrentStep(curr => curr + 1);
      } else {
        const totalScore = newHistory.reduce((a, b) => a + b, 0);
        setLocation("/roommateCompatibility-results?score=" + totalScore);
      }
    }, 300);
  };
  ...
  const question = roommateCompatibilityQuestions[currentStep];
  const progress = Math.round(((currentStep + 1) / roommateCompatibilityQuestions.length) * 100);
  ...
  return (
    <div className="min-h-screen bg-pink-50/30">
      <SEO title="내 룸메이트 성격 유형 테스트" description="룸메이트와의 생활은 생각보다 그 사람의 성격이 큰 영향을 미치잖아? 그럼, 너와 잘 맞는 룸메이트 타입을 알아보자! 솔직히 말해봐~" url="https://mbtifinder.com/roommateCompatibility-test" keywords="PERSONALITY, 내, 룸메이트, 성격, 유형, 테스트" />
      <Navigation />
      {!hasStarted ? (
        <main className="flex-grow max-w-3xl mx-auto w-full px-4 py-12 pb-24 flex flex-col items-center">
          <div className="bg-white/90 backdrop-blur-sm rounded-[2rem] p-8 md:p-12 shadow-xl border border-white/60 flex-col justify-center text-center w-full mb-10">
            ...
            <h1 className="text-3xl md:text-5xl font-extrabold mb-6 tracking-tight text-neutral-900">내 룸메이트 성격 유형 테스트</h1>
            <p className="text-lg md:text-xl text-neutral-500 mb-10 leading-relaxed">너와 잘 맞는 룸메이트는 누구일까?</p>
          ...
          <div className="bg-white/60 backdrop-blur-sm border border-white/60 rounded-2xl p-8 text-left w-full mt-8 prose prose-neutral max-w-none">
            ...
            <p className="text-neutral-600 mb-4">내 룸메이트 성격 유형 테스트는 당신의 심리를 깊이 있게 분석합니다. 룸메이트와의 생활은 생각보다 그 사람의 성격이 큰 영향을 미치잖아? 그럼, 너와 잘 맞는 룸메이트 타입을 알아보자! 솔직히 말해봐~</p>
          ...
          <div className="bg-white/80 backdrop-blur-md shadow-sm sticky top-0 z-20">
            <div className="max-w-2xl mx-auto px-6 py-4">
              ...
              <p className="text-sm font-semibold tracking-wider text-pink-500 mt-3 text-center">
                {currentStep + 1} / {roommateCompatibilityQuestions.length}
              </p>
            </div>
          </div>
              ...
              <div className="p-8 md:p-10 relative z-10">
                <div className="text-center mb-10">
                  ...
                  <h2 className="text-2xl md:text-[1.7rem] font-bold text-neutral-800 leading-snug tracking-tight break-keep">
                    {question?.question || question?.questionText || "내 룸메이트 성격 유형 테스트"}
                  </h2>
                </div>
