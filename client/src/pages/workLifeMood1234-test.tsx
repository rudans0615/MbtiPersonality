...
import { workLifeMood1234Questions, seoArticle } from "@/data/workLifeMood1234Questions";

export default function WorkLifeMood1234Test() {
  ...
  const handleAnswer = (points: number) => {
    ...
    setTimeout(() => {
      if (currentStep < workLifeMood1234Questions.length - 1) {
        setCurrentStep(curr => curr + 1);
      } else {
        const totalScore = newHistory.reduce((a, b) => a + b, 0);
        setLocation("/workLifeMood1234-results?score=" + totalScore);
      }
    }, 300);
  };
  ...
  const question = workLifeMood1234Questions[currentStep];
  const progress = Math.round(((currentStep + 1) / workLifeMood1234Questions.length) * 100);
  ...
  return (
    <div className="min-h-screen bg-pink-50/30">
      <SEO title="당신의 직장 생활 적응력은 몇 점?" description="회사를 다니면서 겪는 다양한 상황에 따라 내 직장 생활 적응력을 알아보는 테스트! 당신은 어떤 유형인지, 또 상사와의 관계는 어떤지 확인해보세요." url="https://mbtifinder.com/workLifeMood1234-test" keywords="CAREER, 당신의, 직장, 생활, 적응력은, 몇, 점?" />
      <Navigation />
      {!hasStarted ? (
        <main className="flex-grow max-w-3xl mx-auto w-full px-4 py-12 pb-24 flex flex-col items-center">
          <div className="bg-white/90 backdrop-blur-sm rounded-[2rem] p-8 md:p-12 shadow-xl border border-white/60 flex-col justify-center text-center w-full mb-10">
            ...
            <h1 className="text-3xl md:text-5xl font-extrabold mb-6 tracking-tight text-neutral-900">당신의 직장 생활 적응력은 몇 점?</h1>
            <p className="text-lg md:text-xl text-neutral-500 mb-10 leading-relaxed">상사와의 케미도 체크해보세요!</p>
          ...
          <div className="bg-white/60 backdrop-blur-sm border border-white/60 rounded-2xl p-8 text-left w-full mt-8 prose prose-neutral max-w-none">
            ...
            <p className="text-neutral-600 mb-4">당신의 직장 생활 적응력은 몇 점?는 당신의 심리를 깊이 있게 분석합니다. 회사를 다니면서 겪는 다양한 상황에 따라 내 직장 생활 적응력을 알아보는 테스트! 당신은 어떤 유형인지, 또 상사와의 관계는 어떤지 확인해보세요.</p>
          ...
          <div className="bg-white/80 backdrop-blur-md shadow-sm sticky top-0 z-20">
            <div className="max-w-2xl mx-auto px-6 py-4">
              ...
              <p className="text-sm font-semibold tracking-wider text-pink-500 mt-3 text-center">
                {currentStep + 1} / {workLifeMood1234Questions.length}
              </p>
            </div>
          </div>
              ...
              <div className="p-8 md:p-10 relative z-10">
                <div className="text-center mb-10">
                  ...
                  <h2 className="text-2xl md:text-[1.7rem] font-bold text-neutral-800 leading-snug tracking-tight break-keep">
                    {question?.question || question?.questionText || "당신의 직장 생활 적응력은 몇 점?"}
                  </h2>
                </div>
