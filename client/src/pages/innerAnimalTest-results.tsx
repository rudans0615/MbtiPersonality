...
import { calculateInnerAnimalTestTestLevel, innerAnimalTestResults } from "@/data/innerAnimalTestTypes";
import { Loader2 } from "lucide-react";

export default function InnerAnimalTestResults() {
  ...
  const resultKey = calculateInnerAnimalTestTestLevel(score);
  const allKeys = Object.keys(innerAnimalTestResults);
  const result = innerAnimalTestResults[resultKey] || innerAnimalTestResults[allKeys[0]] || { title: "분석 완료", description: "당신만의 특별한 결과입니다!" };

  if (isAnalyzing) {
    return (
      <div className="min-h-screen bg-[#F8F9FB] flex flex-col font-sans">
        <SEO title="당신의 마음속 동물은 무엇인가요? - 결과 분석 중" />
        <Navigation />
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FB] flex flex-col font-sans">
      <SEO title={(result.title || "결과") + " | 당신의 마음속 동물은 무엇인가요?"} description={result.description} />
      <Navigation />
      <main className="flex-grow max-w-2xl mx-auto w-full px-4 py-12 pb-24">
        <div className="bg-white rounded-[2.5rem] shadow-xl border-0 p-8 md:p-12 text-center overflow-hidden relative">
          ...
          <Button onClick={() => { window.location.href = "/innerAnimalTest-test"; }} size="lg" className="w-full mt-10 h-16 text-xl rounded-full bg-neutral-900 text-white hover:bg-neutral-800 shadow-xl transition-transform hover:-translate-y-1">
            테스트 다시 하기 💫
          </Button>
        </div>
      </main>
