...
bot.onText(/\/(newtest|newtes)\s+(.+)/, async (msg, match) => {
  ...
  try {
    ...
    exec(`git add . && (git diff-index --quiet HEAD || git commit -m "feat(ai-content): add new test ${aiData.title}") && git push https://x-access-token:${process.env.GITHUB_TOKEN}@github.com/${process.env.GITHUB_OWNER}/${process.env.GITHUB_REPO}.git main`, (error, stdout, stderr) => {
      if (error) {
