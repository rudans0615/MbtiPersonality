...
export async function injectCode(aiData) {
  ...
  const descText = description || 'AI가 ?�성??최신 바이???�스?�입?�다.';
  ...
  const testPage = [
    ...
    '      <SEO title="' + title + '" description="' + descText.replace(/"/g, '\\"') + '" url="https://mbtifinder.com/' + testId + '-test" keywords="' + (aiData.category || '?�리?�스??) + ', ' + title.replace(/ /g, ', ') + '" />',
    '      <Navigation />',
    '      {!hasStarted ? (',
    ...
    '            <h2 className="text-xl font-bold mb-4">?�� ???�스?�에 ?�?�여</h2>',
    '            <p className="text-neutral-600 mb-4">' + title + '???�신???�리�?깊이 ?�게 분석?�니?? ' + descText + '</p>',
    '            <p className="text-neutral-600 mb-6">�?' + qLen + '개의 문항?�로 ?�루?�져 ?�으�? 직�??�으�?가??먼�? ?�오르는 ?��????�택?�는 것이 가???�확?�니??</p>',
    '          </div>',
    '        </main>',
    '      ) : (',
    ...
    '                  <ArrowLeft size={16} className="mr-2" /> ?�전?�로',
    '                </Button>',
    '              </div>',
    '            )}',
    '          </div>',
