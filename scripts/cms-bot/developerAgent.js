...
export async function injectCode(aiData) {
  ...
  const testPage = [
    ...
    '      <SEO title="' + title + '" description="' + descText.replace(/"/g, '\\"') + '" url="https://mbtifinder.com/' + testId + '-test" keywords="' + (aiData.category || '심리테스트') + ', ' + title.replace(/ /g, ', ') + '" />',
    '      <Navigation />',
    '      {!hasStarted ? (',
  ...
  const resultsPage = [
    ...
    '          </div>',
    '          <ShareButtons title="' + title + ' 결과" shareText={`저의 검사 결과는 \\\'${result.title}\\\'입니다!`} url="https://mbtifinder.com/' + testId + '-test" />',
    '          <div className="mt-8 flex justify-center pb-8">',
    '            <Button onClick={() => { window.location.href = "/' + testId + '-test"; }} className="bg-gradient-to-r from-neutral-800 to-neutral-900 text-white px-8 py-6 rounded-full text-lg hover:shadow-lg transition-transform hover:-translate-y-1">',
